...
/**
 * Formats the binary generates, downloaded straight from it.
 *
 * These are links rather than buttons on purpose: the file is produced by the
 * same Rust that `kog scan --graphml` calls, so the page never holds a second
 * implementation of a format it would then have to keep in step.
 */
const EXPORTS = [
  { file: "report.md", label: "Markdown report", note: "The numbers, for a pull request" },
  { file: "graph.graphml", label: "GraphML", note: "Gephi, yEd, Cytoscape" },
  { file: "graph.cypher", label: "Cypher", note: "cypher-shell → Neo4j" },
  { file: "graph.yaml", label: "YAML", note: "The same record as the JSON" },
  { file: "graph.json", label: "JSON", note: "The complete record" },
] as const;
          ...
          <PopoverContent side="top" align="end" className="w-[300px] p-0">
            ...
            <div className="flex flex-col p-1.5">
              <button
                type="button"
                onClick={onExport}
                className="row rounded-md px-2 py-1.5 text-left text-[12px]"
              >
                PNG
              ...
              {/*
                Everything below is generated by the binary, at the same code
                path `kog scan --graphml` runs. Rebuilding them here would be a
                second implementation, and only one of the two would be tested.
              */}
              {EXPORTS.map((format) => (
                <a
                  key={format.file}
                  href={`/export/${format.file}`}
                  download={format.file}
                  className="row rounded-md px-2 py-1.5 text-[12px]"
                >
                  {format.label}
                  <span className="block text-[11px] text-muted-foreground">
                    {format.note}
            ...
            <p className="border-t border-border px-3 py-2 text-[11px] leading-relaxed text-muted-foreground">
              An Obsidian vault is a directory rather than a file:
              <code className="mx-1">kog scan --obsidian dir</code>
              writes one wikilinked note per file.
            </p>
          </PopoverContent>
        </Popover>
